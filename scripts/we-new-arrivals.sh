...
function present() {
  ...
  printf "\033[0mFound \033[0;33m%d\033[0m entries for \033[0;33m%s\033[0m (%s)\n\n" "$entries" "$date_posted" "$since_posted"
  ...
  for n in $(seq 0 $((entries - 1))); do
    ...
    if [[ -n "$highlight" ]] && [[ "$title_lc" == *"$highlight_lc"* ]]; then
      title="$(printf "👉  %s  👈" "$(perl -pe "s/($highlight_lc)/\\033[7m\1\\033[27m/gi" <<<"$title")")"
    fi

    printf "%${digits}d. \033[0;93m%s\033[0m\n%${digits}s  \033[0;37m%s\033[0m\n" "$((n + 1))" "$title" "" "$(extract_href "$n")"
  done
}
